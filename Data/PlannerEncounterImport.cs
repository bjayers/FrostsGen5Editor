...
namespace NewEditor.Data
{
    public class PlannerEncounterImport
    {
        ...
        static readonly Dictionary<string, string> LocationToPoolName = new Dictionary<string, string>
        {
            ...
            { "virbank-city", "Virbank City" },
            { "humilau-city", "Humilau City" },
            { "dreamyard", "Dreamyard 1" },
            { "dreamyard-b1f", "Dreamyard 2" },
            { "pinwheel-outside", "Pinwheel Forest 1" },
            { "pinwheel-inside", "Pinwheel Forest 2" },
            { "desert-resort-entrance", "Desert Resort 1" },
            { "desert-resort", "Desert Resort 2" },
            { "relic-castle-1f", "Relic Castle 1" },
            { "relic-castle-b1f", "Relic Castle 2" },
            { "relic-castle-b2f", "Relic Castle 3" },
            { "relic-castle-lowest", "Relic Castle 4" },
            { "chargestone-1f", "Chargestone Cave 1" },
            { "chargestone-b1f", "Chargestone Cave 2" },
            { "chargestone-b2f", "Chargestone Cave 3" },
            { "twist-1f", "Twist Mountain 1" },
            { "twist-b1f", "Twist Mountain 2" },
            { "twist-b2f", "Twist Mountain 3" },
            { "twist-ice", "Twist Mountain 4" },
            { "dragonspiral-outside", "Dragonspiral Tower 1" },
            { "dragonspiral-entrance", "Dragonspiral Tower 2" },
            { "dragonspiral-1f", "Dragonspiral Tower 3" },
            { "dragonspiral-2f", "Dragonspiral Tower 4" },
            { "dragonspiral-interior", "Dragonspiral Tower 4" },
            { "victory-grove", "Victory Road" },
            { "victory-entrance", "Victory Road 1" },
            { "victory-1f", "Victory Road 2" },
            { "victory-2f", "Victory Road 3" },
            { "victory-3f", "Victory Road 4" },
            { "victory-ruins", "Victory Road 5" },
            { "victory-7f", "Victory Road 7" },
            { "victory-waterfall", "Victory Road 8" },
            { "giant-chasm-outside", "Giant Chasm 1" },
            { "giant-chasm-entrance", "Giant Chasm 2" },
            { "giant-chasm-forest", "Giant Chasm 3" },
            { "giant-chasm-cave", "Giant Chasm 4" },
            { "castelia-sewers", "Castelia Sewers 1" },
            { "castelia-sewers-b1f", "Castelia Sewers 2" },
            { "p2-laboratory", "P2 Laboratory" },
            { "undella-bay", "Undella Bay" },
            { "floccesy-ranch-outer", "Floccesy Ranch 1" },
            { "floccesy-ranch-inner", "Floccesy Ranch 2" },
            { "virbank-complex-outer", "Virbank Complex 1" },
            { "virbank-complex-inner", "Virbank Complex 2" },
            { "reversal-exterior", "Reverse Mountain 1" },
            { "reversal-1f", "Reverse Mountain 2" },
            { "reversal-b1f", "Reverse Mountain 3" },
            { "reversal-interior-a", "Reverse Mountain 4" },
            { "reversal-interior-b", "Reverse Mountain 5" },
            { "strange-house-1f", "Stranger's House 1" },
            { "strange-house-b1f", "Stranger's House 2" },
            { "relic-passage-sewers", "Relic Passage 1" },
            { "relic-passage-pwt", "Relic Passage 2" },
            { "relic-passage-castle", "Relic Passage 3" },
            { "clay-tunnel", "Clay Road 1" },
            { "underground-ruins", "Underground Ruins 1" },
            { "rocky-mountain-room", "Rocky Mountain Room" },
            { "glacier-room", "Glacier Room" },
            { "iron-room", "Iron Room" },
            { "seaside-cave-1f", "Seaside Grotto 1" },
            { "seaside-cave-b1f", "Seaside Grotto 2" },
            ...
            { "route-2", "Route 2" },
            { "route-3", "Route 3" },
            { "wellspring-1f", "Wellspring Cave 1" },
            { "wellspring-b1f", "Wellspring Cave 2" },
            { "route-4", "Route 4 1" },
            { "route-5", "Route 5" },
            { "route-6", "Route 6" },
            { "mistralton-cave-1f", "Mistralton Cave 1" },
            { "mistralton-cave-2f", "Mistralton Cave 2" },
            { "guidance-chamber", "Guidance Chamber" },
            { "route-7", "Route 7" },
            { "celestial-2f", "Celestial Tower 1" },
            { "celestial-3f", "Celestial Tower 2" },
            { "celestial-4f", "Celestial Tower 3" },
            { "celestial-5f", "Celestial Tower 4" },
            { "route-8", "Route 8" },
        ...
        public static Result Import(string jsonText, EncounterNARC narc)
        {
            ...
            using (JsonDocument doc = JsonDocument.Parse(jsonText))
            {
                ...
                foreach (var p in placements.EnumerateArray())
                {
                    ...
                    if (!LocationToPoolName.ContainsKey(locId))
                    {
                        unmappedLoc.Add(locId);
                        continue;
                    }
                ...
                foreach (var kv in tables)
                {
                    ...
                    if (!nameToId.ContainsKey(poolName))
                    {
                        result.MissingPools.Add(poolName);
                        continue;
                    }
                    int nameId = nameToId[poolName];
                    ...
                    foreach (var slotKv in kv.Value)
                    {
                        ...
                        {
                            result.Warnings.Add(locId + " " + type + " slot " + slot + " out of range");
                            continue;
                        }
        ...
        static EncounterEntry FindOrCreateSeasonEntry(EncounterNARC narc, int nameId, int season, Result result)
        {
            ...
            if (mains.Count == 0)
            {
                result.MissingPools.Add("nameID " + nameId);
                return null;
            }
